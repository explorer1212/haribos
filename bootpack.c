...
void HariMain(void)
{
	...
	enable_mouse(&fifo, 512, &mdec); /* use the same buffer as the keyboard */
	io_out8(PIC0_IMR, 0xf8); /* (11111000) open interrupt, 20, 21, 22 */
	io_out8(PIC1_IMR, 0xef); /* (11101111) open interrupt, 2c */
...
/**
 * @description: write s to the sheet
 * @param {SHEET} *sht
 * @param {int} x		coordinate
 * @param {int} y
 * @param {int} c		color
 * @param {int} b		background
 * @param {char} *s		string
 * @param {int} l 		length of s
 * @return {*}
 */
void putfonts8_asc_sht(struct SHEET *sht, int x, int y, int c, int b, char *s, int l)
{
